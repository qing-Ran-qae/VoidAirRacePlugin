...
public class VoidAirRaceBootstrap implements PluginBootstrap {
    @Override
    public void bootstrap(@NotNull BootstrapContext context) {
        ...
        List<Class<? extends BootstrapModule>> modules = ClassScanner.scanSubclasses(
                classLoader, codeSourceUrl, BootstrapModule.class, "io.github.qingranqae.voidairrace"
        );
        logger.debug("共扫描到 {} 个 BootstrapModule 实现：", modules.size());
        for (Class<? extends BootstrapModule> module : modules) {
            logger.debug(module.getName());
        }

        // 实例化并调用
        for (Class<? extends BootstrapModule> moduleClass : modules) {
            try {
