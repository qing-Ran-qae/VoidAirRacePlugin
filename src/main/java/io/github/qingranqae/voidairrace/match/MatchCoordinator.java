...
public class MatchCoordinator {
     ...
     * */
    public void startMatch(MatchConfig matchConfig) throws IllegalStateException, MapNotPlayableException {
        if (this.matchState != MatchState.SCHEDULED) {
            throw new IllegalStateException("比赛已在进行，无法重复开始");
        }
     ...
     * */
    public void stopMatch(boolean mandatory) throws IllegalStateException {
        if (
                this.matchState != MatchState.IN_PROGRESS
                && !mandatory
        ) { throw new IllegalStateException("当前比赛状态不是“进行中”，无法结束比赛"); }
