...
public class Config {
     ...
     *
     * @return 配置管理器实例
     * @throws IllegalStateException 如果实例尚未初始化（未调用 {@link #getInstance(VoidAirRace)}）
     */
     ...
     * @return 配置管理器实例
     */
    public static Config getInstance(VoidAirRace mainClass) {
        if (instance == null) {
     ...
     * @param mainClass 插件主类实例
     */
    private Config(VoidAirRace mainClass) {
    ...
    public ObservableYamlConfiguration getConfig(ConfigFiles config) {
        ...
        if (!osFile.isFile()) {
            ...
            try {
                ...
                logger.info("已从 jar 复制配置文件: " + osFile.getAbsolutePath());
        ...
        for (int i = 1; i <= maxRetries; i++) {
            try {
                if (osFile.createNewFile()) {
                    logger.info("成功创建空配置文件: " + osFile.getAbsolutePath());
                    return true;
                } else {
