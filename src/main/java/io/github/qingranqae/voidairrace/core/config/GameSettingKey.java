...
/**
 * 游戏设置配置文件（gameSetting.yml）中定义的配置键枚举。
 * 每个枚举常量对应一个配置项，包含其在文件中的路径和数据类型。
 */
public enum GameSettingKey implements ConfigKey {
    /** 当前选中的比赛地图ID。 */
    SELECTED_MAP_ID("selectedMapId", String.class),

    /** 比赛持续时间，单位为 tick（20 ticks = 1秒）。 */
    MATCH_DURATION("matchDuration", Integer.class),

    /** 玩家出生点位置的完整配置映射。 */
    SPAWN_LOCATION("SpawnLocation", Map.class),

    /** 出生点所在的世界名称。 */
    SPAWN_WORLD_NAME(SPAWN_LOCATION.getPath() + ".worldName", String.class),
     ...
     * @param type 配置项数据类型
     */
    GameSettingKey(String path, Class<?> type) {
        this.path = path;
        this.type = type;
    }
