...
public class Config {
        ...
        // 确保配置文件存在，不存在将依次尝试从 jar 复制默认值或创建空文件
        {
            if (!osFile.isFile()) {
                ...
                try {
                    ...
                    if (!createEmptyYmlFile(osFile, configName)) {
     ...
     * @return 是否成功创建
     */
    private boolean createEmptyYmlFile(File osFile, String configName) {
        int maxRetries = 3;
        for (int i = 1; i <= maxRetries; i++) {
            try {
    ...
    /**
     * 将插件 jar 中的默认配置 A 融合到已加载的配置对象 B中（补充缺失的字段）
     *
     * @param source 配置 A
     * @param target 配置 B
     */
    private void mergeDefaults(ConfigFile source, YamlConfig target) {
        ...
        try (InputStream in = VoidAirRace.getInstance().getClass().getClassLoader().getResourceAsStream(resourceName)) {
            ...
            for (String key : defaultConfig.getKeys(true)) {
                if (!target.contains(key)) {
                    ...
                    logger.fine("为配置文件 '" + source.fileName() + "' 补充缺失字段: " + key);
                }
            }
        } catch (IOException e) {
